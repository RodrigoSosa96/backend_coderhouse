...
abstract class Database {
	abstract readonly _knex: Knex

	public table: string
	constructor(table: string) {
		this.table = table
	}
	abstract _checkTable(): Promise<any>
...
class Producto extends Database {
	public readonly _knex = knex(options.mariaDB)
	async _checkTable(): Promise<any> {
		try {
...
class Chat extends Database {
	public readonly _knex = knex(options.sqlite3)
	async _checkTable(): Promise<any> {
		try {
			const exist = await this._knex.schema.hasTable(this.table)
			if (exist) return
...
// productos.deleteItem(2).then(res => console.log(res))
// productos.getItems().then(res => console.log(res))
// productos.newItems({codigo: "", descripcion: ""})
